...
*Setup instructions for a gitian build of cobrax using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the cobrax
...
- [Building cobrax](#building-cobrax)
...
Clone the git repositories for cobrax and gitian and then checkout the cobrax version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cobraxcoin/cobrax-core.git
cd cobrax-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cobrax-core repository
...
To build cobrax (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cobrax-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cobrax-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cobraxcoin/cobrax-core
...
./bin/gbuild --commit cobrax=${COMMIT} --url cobrax=${URL} ../cobrax/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cobrax=${COMMIT} --url cobrax=${URL} ../cobrax/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cobrax=${COMMIT} --url cobrax=${URL} ../cobrax/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cobrax-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cobrax-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cobrax-build.assert
```
...
[cobraxcoin/gitian.sigs](https://github.com/cobraxcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
